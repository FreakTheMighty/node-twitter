...
Twitter.prototype.generateCacheKey = function(url, params){
	if(this.options.cache){
		var md5sum = crypto.createHash('md5');
		var _this=this;
		['consumer_key','consumer_secret','access_token_key','access_token_secret']
			.forEach(function(opt){
				md5sum.update(_this.options[opt]);
			})
		md5sum.update(url);
		md5sum.update(JSON.stringify(params));
...
Twitter.prototype.cacheResults = function(key, data){
	if(this.options.cache && key){
		var string = JSON.stringify(data);
		var namespaced = 'twitter:'+key;
		this.options.cache.set(namespaced, string);
		this.option.cache.expire(namespaced, 60);
	}
}

Twitter.prototype.getCache = function(key, cacheFound, onNoCache){
	if(key){
		this.options.cache.get('twitter:'+key, function(err, data){
			if(data){
...
Twitter.prototype.get = function(url, params, callback) {
	var cacheKey = this.generateCacheKey(url, params);
	...
	var _this = this;
	var hitTwitter = function(){
			_this.oauth.get(url,
			_this.options.access_token_key,
			_this.options.access_token_secret,
		function(error, data, response) {
			if (error) {
				...
				err.data = error.data;
				_this.cacheResults(cacheKey, err);
				callback(err);
			} else {
				try {
					var json = JSON.parse(data);
					_this.cacheResults(cacheKey, json);
					callback(json);
				} catch(err) {
					_this.cacheResults(cacheKey, err);
					callback(err);
	...
	this.getCache(cacheKey, callback, hitTwitter);
	return this;
};
